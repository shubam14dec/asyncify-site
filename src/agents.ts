...
const PIN_HEIGHTS = 3.0; // SETTLED (user call, 2026-08-04): rode 2.0/2.6/2.7/2.8/3.0 -- 3.0 is the keeper
const TL_END = 80;
...
const CAM_RETURN = 0.9; // the user's ruling: the pull-back IS the push-in reversed -- same span, same ease
...
/* The per-row aim points retired with them: the camera now takes ONE shot
   (masthead + full checklist, fitted) for all three asides. */
...
export function createAgentsScene(): AgentsScene {
  ...
  /** ONE framing for all three asides: the whole rail block -- kicker, head,
   *  full checklist -- fitted top-to-bottom. THE POSE IS MEASURED ONCE, WITH
   *  THE CAMERA HOME, AND CACHED. This is not an optimisation, it is the
   *  bug fix (user catch: "the return isn't smooth"): getBoundingClientRect
   *  returns TRANSFORMED geometry, so a pull-back that measured at its own
   *  first render -- mid-zoom -- launched from garbage coordinates and
   *  lurched home. The push-in always renders first (timeline children
   *  render in time order, and restState parks the camera at home), so the
   *  first evaluation is guaranteed untransformed; every later reader gets
   *  the cache, and both directions travel the same line. Stale on resize
   *  mid-scene -- accepted and documented: a resize re-fires the stills
   *  decision anyway. */
  const CAM_MARGIN = 20;
  let camPose: { s: number; x: number; y: number } | null = null;
  const pose = () => {
    if (!camPose) {
      const c = cam.getBoundingClientRect();
      const ttl = title.getBoundingClientRect();
      const k = checklist.getBoundingClientRect();
      const top = ttl.top - c.top;
      const left = Math.min(ttl.left, k.left) - c.left;
      const height = k.bottom - ttl.top;
      const s = Math.min(MEM_SCALE, (c.height - 2 * CAM_MARGIN) / height);
      camPose = { s, x: CAM_MARGIN - s * left, y: CAM_MARGIN - s * top };
    }
    return camPose;
  };
  const camS = () => pose().s;
  ...
  function buildBridge(): void {
    ...
    if (svgB && bwire && bdot && eraPhrase && sectionB && bridgeLines.length >= 3) {
      ...
      /* THE EXIT IS THE ENTRY'S MIRROR (user call, insisted on: "it should
         be the same curve or the angle... so that symmetry is maintained").
         The dive's recipe is: control along the incoming tangent at HALF the
         travel, control on the target line 170px before the endpoint. The
         exit is that recipe reflected across the underline: 170px back-off
         leaving the rule (mirror of the arrival's 170), and a control at
    ...
    if (bwire && bdot && totalLen && fractions.length === 4) {
      ...
      if (eraPhrase) {
        /* ::selection paints TEXT too (user catch): the site's selection rule
           is green behind, full --text ink on top. The words whiten across
           the same window the band sweeps them, ease none to ride the dot. */
        tl.fromTo(
          eraPhrase,
          { color: "#6e6e6e" },
          { color: "#ededed", duration: Math.max(0.4, tU1 - t0 - 0.1), ease: "none", immediateRender: false },
          t0 + 0.1,
        );
      }
  ...
  function buildScrub(): void {
       ...
       scrubbing back re-dims, re-scales and re-folds in reverse order. */
    {
      ...
      ft(cam, { scale: 1, x: 0, y: 0 }, { scale: camS, x: camX, y: camY, duration: 0.9, ease: "power2.inOut", transformOrigin: "0px 0px" }, MEM_IN);
      ...
      ft(cam, { scale: camS, x: camX, y: camY }, { scale: 1, x: 0, y: 0, duration: CAM_RETURN, ease: "power2.inOut", transformOrigin: "0px 0px" }, MEM_FOCUS_OUT);
       ...
       other than row 3 recede this time. */
    {
      ...
      ft(cam, { scale: 1, x: 0, y: 0 }, { scale: camS, x: camX, y: camY, duration: 0.9, ease: "power2.inOut", transformOrigin: "0px 0px" }, GRD_IN);
      ...
      ft(cam, { scale: camS, x: camX, y: camY }, { scale: 1, x: 0, y: 0, duration: CAM_RETURN, ease: "power2.inOut", transformOrigin: "0px 0px" }, GRD_FOCUS_OUT);
       ...
       row's pivot. */
    {
      ...
      ft(cam, { scale: 1, x: 0, y: 0 }, { scale: camS, x: camX, y: camY, duration: 0.9, ease: "power2.inOut", transformOrigin: "0px 0px" }, GUA_IN);
      ...
      ft(cam, { scale: camS, x: camX, y: camY }, { scale: 1, x: 0, y: 0, duration: CAM_RETURN, ease: "power2.inOut", transformOrigin: "0px 0px" }, GUA_FOCUS_OUT);
  ...
  function wireTicket(): void {
    ...
    const tearFibersIn = (): void => {
      ...
      /* A whisper of jitter (user-tuned across four values: 1.4 rattled,
         0.35 still shook, 0 was too clean -- 0.12 is the hand's tremor). */
      const roughRun =
    ...
    tktPaper.addEventListener("click", () => {
      ...
      /* THE HINGE TRAVELS WITH THE TEAR FRONT (user catch: the whole sheet
         was hanging from the first instant -- even the part not yet torn.
         Only as much paper may hang as has been cut). Four chained segments
         whose pivot walks rightward with the front -- 18% -> 42% -> 64% ->
         86% of the sheet -- each hanging deeper; smoothOrigin compensates
         every pivot hand-off, so the walk is seamless. Early on, only the
