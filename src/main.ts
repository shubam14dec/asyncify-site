...
if (window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
  ...
  const showCue = (): void => {
    ...
    cue.append(label);
    // The mouse, BELOW the words (user rode both layouts and picked the
    // stack): a 1px outline body with a green wheel tick, bobbing gently
    // downward (CSS animation) — the gesture the reader is being asked to
    // make. The hero's own tiny hint keeps its no-mouse stance; this
    // louder guided-landing cue speaks plainer on purpose.
